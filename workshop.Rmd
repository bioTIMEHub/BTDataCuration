...
And now we switch over to Excel. Our database has several data tables for each dataset and they're all linked and cross-referenced by the StudyID. The raw data you just prepared is one data table, but we now have to fill in other data tables on metaData. Most of the metadata will either be found in the source paper or provided by the dataset contributor.

One that we may have to calculate ourselves are the central coordinates (`CentralLatitude, CentralLongitude`) and sampling area. For datasets grained to one location, these fields should already be provided. For datasets where authors provide us with multiple locations, we use R spatial analysis to calculate the central point and area. This can be easily done using a few lines of code to create a convex hull with the multiple locations.
...
#transform back
points204 <- spTransform(points204, CRS('+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs'))
convhull204<-gConvexHull(points204)
...
centroid204@coords
clipr::write_clip((centroid204@coords[c(2,1)])) # get centroid coordinates
